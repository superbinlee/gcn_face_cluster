...
class Feeder(data.Dataset):
    ...
    def __init__(self, feat_path, knn_graph_path, label_path, seed=1, k_at_hop=[200, 5], active_connection=5, train=True):
        np.random.seed(seed)
        random.seed(seed)
    ...
    def __getitem__(self, index):
        ...
        unique_nodes_list = torch.cat([unique_nodes_list, torch.zeros(max_num_nodes - num_nodes)], dim=0)
