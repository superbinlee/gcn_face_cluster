...
def main(args):
    ...
    valset = Feeder(args.val_feat_path,
                    ...
                    train=False)
    valloader = DataLoader(valset, batch_size=args.batch_size, num_workers=args.workers, shuffle=False, pin_memory=True)
    ...
    for i, ((feat, adj, cid, h1id, node_list), gtmat) in enumerate(loader):
        ...
        if i % args.print_freq == 0:
            print('[{0}/{1}]\t'
                  ...
                  'Recall {recalls.val:.3f} ({recalls.avg:.3f})'.format(i, len(loader),
...
if __name__ == '__main__':
    ...
    # Validation args 
    parser.add_argument('--val_feat_path', type=str, metavar='PATH', default=osp.join(working_dir, '../facedata/1024.fea.npy'))
    parser.add_argument('--val_knn_graph_path', type=str, metavar='PATH', default=osp.join(working_dir, '../facedata/knn.graph.1024.bf.npy'))
    parser.add_argument('--val_label_path', type=str, metavar='PATH', default=osp.join(working_dir, '../facedata/1024.labels.npy'))
